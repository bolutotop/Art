...
export function Toast({ toasts, onClose }: ToastProps) {
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 max-w-sm w-full">
      {toasts.map((toast) => {
        const bg = toast.type === "success" ? "bg-emerald-600" : toast.type === "error" ? "bg-red-600" : "bg-art-charcoal";
        const Icon = () => {
          ...
          return <div className="w-4 h-4 border-2 border-art-stone border-t-white rounded-full animate-spin"></div>;
        };
